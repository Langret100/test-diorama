...
// Color management / tone mapping (helps keep the soft, pastel look consistent)
 ...
 *  - public/config/scene.json (remove ceiling, remove kirby)
 *  - public/config/monitor.json (monitor slideshow images)
 */
...
const sceneConfigPromise = fetch("/config/scene.json", {
  cache: "no-store",
})
  .then((r) => (r.ok ? r.json() : null))
  .catch(() => null);

const monitorConfigPromise = fetch("/config/monitor.json", {
  cache: "no-store",
})
  .then((r) => (r.ok ? r.json() : null))
  .catch(() => null);
...
// Monitor screen material (video by default, can switch to slideshow via /config/monitor.json)
const screenMaterial = new THREE.ShaderMaterial({
  uniforms: {
    uTex1: { value: videoTexture },
    uTex2: { value: videoTexture },
    uMix: { value: 0 },
    uOpacity: { value: 0.92 },
  },
  transparent: true,
...
const applySceneToggles = (root, cfg) => {
  const merged = {
    ...DEFAULT_SCENE_CONFIG,
    ...(cfg || {}),
    ceilingAutoDetect: {
      ...DEFAULT_SCENE_CONFIG.ceilingAutoDetect,
      ...((cfg || {}).ceilingAutoDetect || {}),
    },
  };

  // Remove Kirby (requested)
  if (merged.removeKirby) {
    root.traverse((child) => {
      if (child.isMesh && child.name.includes("Kirby")) {
        child.visible = false;
      }
    });
  }

  // Best-effort ceiling removal (auto-detect a large, thin top plane)
  if (merged.removeCeiling) {
    ...
    root.traverse((child) => {
      ...
      const looksLikeCeiling =
        center.y >= merged.ceilingAutoDetect.minCenterY &&
        size.x >= merged.ceilingAutoDetect.minXZ &&
        size.z >= merged.ceilingAutoDetect.minXZ &&
        size.y <= merged.ceilingAutoDetect.maxYThickness;
...
const initMonitorSlideshow = async () => {
  const cfg = await monitorConfigPromise;
  if (!cfg || !cfg.enabled || !screenMesh) return;

  const images = Array.isArray(cfg.images) ? cfg.images.filter(Boolean) : [];
  if (images.length < 2) return;

  // Stop video once slideshow is active
  try {
    videoElement.pause();
  } catch {}

  const intervalMs = Math.max(800, Number(cfg.intervalMs) || 2500);
  const fadeMs = Math.max(200, Number(cfg.fadeMs) || 600);

  // Preload first two
  let idx = 0;
  let texA = await loadScreenTexture(images[idx % images.length]);
  let texB = await loadScreenTexture(images[(idx + 1) % images.length]);
  screenMaterial.uniforms.uTex1.value = texA;
  screenMaterial.uniforms.uTex2.value = texB;
  screenMaterial.uniforms.uMix.value = 0;

  const tick = async () => {
    // Crossfade A -> B
    gsap.to(screenMaterial.uniforms.uMix, {
      value: 1,
      ...
      onComplete: async () => {
        ...
        // Load next into B
        const nextPath = images[(idx + 1) % images.length];
        try {
          texB = await loadScreenTexture(nextPath);
          screenMaterial.uniforms.uTex2.value = texB;
        } catch {
          // fallback: keep current
          screenMaterial.uniforms.uTex2.value = texA;
        }
...
const useOriginalMeshObjects = ["Bulb", "Cactus"];
...
loader.load("/models/Room_Portfolio.glb", async (glb) => {
  ...
  // Apply optional scene toggles (ceiling, kirby)
  const sceneCfg = await sceneConfigPromise;
  applySceneToggles(glb.scene, sceneCfg);

  scene.add(glb.scene);
  // Enable monitor slideshow if configured
  initMonitorSlideshow();
});
...
function handleRaycasterInteraction() {
  if (currentIntersects.length > 0) {
    // Prefer piano keys if multiple objects overlap under the pointer
    let hitbox = null;
    let object = null;
    for (const i of currentIntersects) {
      const candidate = hitboxToObjectMap.get(i.object);
      if (!candidate) continue;
      if (candidate.name.includes("Key")) {
        hitbox = i.object;
        object = candidate;
        break;
      }
    ...
    if (!object) {
      hitbox = currentIntersects[0].object;
      object = hitboxToObjectMap.get(hitbox);
    }
    if (!object) return;
...
function playHoverAnimation(objectHitbox, isHovering) {
  // Softer hover response (requested)
  let scale = 1.2;
...
const render = (timestamp) => {
  ...
  if (chairTop) {
    // Subtle, slow wobble (requested)
    const t = timestamp * 0.001;
    const swing = Math.sin(t * 0.18);
    const rotAmp = Math.PI / 64;
    const posAmp = 0.025;

    chairTop.rotation.y = chairTop.userData.initialRotation.y + rotAmp * swing;
    chairTop.position.x = chairTop.userData.initialPosition.x + posAmp * swing;
  }
